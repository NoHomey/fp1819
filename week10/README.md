...
9. Дефинирайте функция `rotate xs i`, която премества всеки елемент от списъка `xs` с `i` позиции наляво
...
10. Използвайки някой `fold` дефинирайте (http://www.cantab.net/users/antoni.diller/haskell/questions/quest06.pdf):
  ...
  - `remdups  xs` - премахва последователните повторения да даден елемент в списък
    ```haskell
    remdups [1, 2, 2, 3, 3, 3, 1, 1]    -- връща [1, 2, 3, 1]
    ```
  - `inits xs` - връща всички представи на списъка `xs`
    ```haskell
    intis "ate" = [[], "a", "at", "ate"]
    ```
  - `rotate xs` - от задача 9  
...
12. Дефинирайте функция `compose xs` със следната типова дефиниция:
  ...
  ```haskell
  compose [(1, 2), (7, 11), (8, 10)] [(2, 3), (11, 14)]
  -- връща [(1, 3), (7, 14)].
  ```
...
14. Да се дефинира функция `findMax f start end`, която по дадена двуместна числова функция `f` и цели числа `start` и `намира` най-голямата стойност от вида `f(i, f(i+1, … f(j-1, j))` за `start ≤ i < j ≤ end`.
